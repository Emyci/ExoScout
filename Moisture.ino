...
int moisture1_pin = A4;            //moisture 
int moisture2_pin = A3;
float moi1, moi2;

float dp, pp;      //pressure
...
void loop() 
{
  ...
  moi1=analogRead(moisture1_pin);
  float moist1=moisture1_moi1_to_moist1(moi1);
  moi2=analogRead(moisture2_pin);
  float moist2=moisture2_moi2_to_moist2(moi2);
  String dataString = "";
  ...
  if(dp<1)
  {
     dataString+= String(moist1);
     dataString+= (" %, ");
     dataString+= String(moist2);
     dataString+= (" %");
  }
